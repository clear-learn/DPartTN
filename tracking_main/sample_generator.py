...
def gen_samples(generator, bbox, n, overlap_range=None, scale_range=None,grid_size=None):
...
class SampleGenerator():
    def __init__(self, type, img_size, trans_f=1, scale_f=1, aspect_f=None, valid=False):
        ...
        self.valid = valid
        self.scale = [0.5, 0.7, 1.0, 1.5]
        self.wh_sigma = 0.3
        self.grid_num = 7
        #self.scal_valid = scal_valid
    ...
    def __call__(self, bb, n):
        ...
        # (center_x, center_y, w, h)
        sample = np.array([bb[0] + bb[2] / 2, bb[1] + bb[3] / 2, bb[2], bb[3]], dtype='float32')
        #if self.scal_valid is not None:
            #sample[2] = sample[2]
            #sample[3] = sample[3]
        samples = np.tile(sample[None,:],(n,1))
        ...
        if self.type=='gaussian':
            ...
            #if self.scal_valid  is not None:
                #samples[:,2:] = samples[:,2:]
            #elif self.scal_valid == True:
            samples[:,2:] *= self.scale_f ** np.clip(0.5*np.random.randn(n,1),-1,1)

        elif self.type=='sample':
            samples[:,:2] += 1.5 * np.mean(bb[2:]) * np.clip(0.5*np.random.randn(n,2),-1,1)
            #if self.scal_valid  is not None:
                #samples[:,2:] = samples[:,2:]
            #elif self.scal_valid == True:
            samples[:, 2:] *= self.scale_f ** np.clip(0.5 * np.random.randn(n, 1), -1, 1)
