...
class MDNet(nn.Module):
    ...
    def delete_branch(self, k):
        for del_bran in range(k.shape[0]):
            del self.branches[k[del_bran]]

    def plus_branch(self, k):
        for del_bran in range(k):
            self.branches.append(nn.Sequential(nn.Dropout(0.5),
                                               nn.Linear(512, 2)))
        self.build_param_dict()
