from util_dataset_COCO_nowhole_general import *
import network_256concat as network
...
##################### Load Train Data #####################
train_dataset = COCODataset(root=file_root,data_dir= 'train2017', list_file=['train_gt_all_more.txt'], train=True,transform=transforms.ToTensor())
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=1)


test_dataset = COCODataset(root=file_root, data_dir= 'val2017',list_file=['val_gt_all_more.txt'], train=False,transform=transforms.ToTensor())
...
for epoch in range(num_epochs):
    ...
    tr_acc.append(total_acc / (i + 1))
    tr_loss.append(total_loss / (i + 1))
    ...
    te_acc.append(vald_acc)
    te_loss.append(validation_loss)

    file_name = '256_concat.pth'
    save_checkpoint({
        'epoch': epoch + 1,
        'arch': 'penet',
